...
keywords: Documentation, MRZ Scanner, Dynamsoft MRZ Scanner for Web,
...
- UNPKG
  ...
  ```html
  <script src="https://unpkg.com/dynamsoft-mrz-scanner@2.0.0/dist/mrz.bundle.js"></script>
  ```
...
When using a package manager like **npm** or **yarn**, you will most probably need to specify the location of the engine files, which will be the CDN link.

```ts
// in framework like React, Angular, or Vue
<insert what the code will look like in MRZScannerConfig when using React>
```
```js
// in vanilla js
const mrzscanner = new Dynamsoft.MRZScanner({
    license: "YOUR_LICENSE_KEY_HERE",
    engineResourcePaths: "https://cdn.jsdelivr.net/npm/",
});
```

Below is the complete *Hello World* sample page that uses the precompiled script via a CDN.
...
>Note:
>
> This code is identical to the Hello World file mentioned in *Build from Source*, except for the script source of course.
>
> Please do not forget to replace 'YOUR-LICENSE-KEY-HERE' with your own license key, whether it is trial or full.
...
Alternatively, you can use other methods like `IIS` or `Apache` to serve the project, though that won't cover those here for brevity.
...
While using the pre-compiled script is the easiest way to get started with the MRZ Scanner, we do also allow the developer to build the scanner themselves from the source files. The advantage of doing this is that it allows the developer to do some deep customization of the scanner if they are familiar with using the foundational products **Dynamsoft Label Recognizer**, **Dynamsoft Code Parser**, and the **Dynamsoft Camera Enhancer**.

This method requires retrieving the **MRZ Scanner for Web** source files from its [Github repo], compiles them into a distributable package, and then runs a *ready-made* Hello World sample page that is already included in the repo.
